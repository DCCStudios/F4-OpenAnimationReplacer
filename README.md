...
| `preserveExtractedMotion` | bool | false | Preserve a validated `hkaDefaultAnimatedReferenceFrame` from a replacement `.hkx` when building the runtime clone. Use for donors authored with root motion; pose-only replacements should leave this disabled. Reload configs after changing it |
