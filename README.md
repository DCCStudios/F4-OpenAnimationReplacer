...
| `preserveExtractedMotion` | bool | false | Keep the replacement `.hkx`'s root-motion reference frame (`hkaDefaultAnimatedReferenceFrame`) when building the runtime clone, after the packfile identifies it and the object validates. Enable only for donors authored with root motion; pose-only replacements should leave it off. Reload configs after changing it |
